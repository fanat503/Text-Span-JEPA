...
class TrainingStability:
    ...
    def convergence_curve(checkpoint_representations, final_representations=None):
        ...
        N = final_representations.size(0) if final_representations.dim() == 2 else final_representations.size(0)
        cka_values = []
        ...
        return {
            ...
            'mean_consecutive_change': sum(consecutive_cka) / len(consecutive_cka) if consecutive_cka else 0.0,
...
class LossStability:
    ...
    def compute(loss_values, window_size=10):
        ...
        ).squeeze()[:len(losses)]  # Truncate to original length
...
class EarlyStoppingAdvantage:
    ...
    @staticmethod
    @torch.no_grad()
    def compute(checkpoint_representations, checkpoint_fractions,
                threshold=0.9, probe_fn=None, labels=None):
    ...
    @staticmethod
    def compare(jepa_checkpoints, baseline_checkpoints,
                jepa_fractions, baseline_fractions, threshold=0.9):
        """Compare early stopping advantage between JEPA and baseline."""
        jepa_early = EarlyStoppingAdvantage.compute(
            jepa_checkpoints, jepa_fractions, threshold)
        ...
        return {
            ...
            'jepa_converges_earlier': jepa_early['early_stop_fraction'] < baseline_early['early_stop_fraction'],
            'compute_saving': baseline_early['early_stop_fraction'] - jepa_early['early_stop_fraction'],
        }
...
class CheckpointConsistency:
    ...
    def cross_seed_cka(seed_representations):
        ...
        return {
            ...
            'std_pairwise_cka': (sum((c - sum(pairwise_cka) / len(pairwise_cka)) ** 2
                                     for c in pairwise_cka) / max(len(pairwise_cka), 1)) ** 0.5
            if pairwise_cka else 0.0,
            'n_seeds': n_seeds,
    ...
    def compare(jepa_seed_reps, baseline_seed_reps):
        ...
        return {
            ...
            'jepa_more_consistent': jepa_cons['mean_pairwise_cka'] > baseline_cons['mean_pairwise_cka'],
        }
