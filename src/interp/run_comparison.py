...
def load_model(ckpt_path, model_type='jepa', device='cpu'):
    ...
    if model_type == 'jepa':
        ...
        ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
        model = MLMBaseline(vocab_size=50304, max_seq_len=512, embed_dim=768, depth=12, num_heads=12)
        model.load_state_dict(ckpt.get('model', {}))
    elif model_type == 'data2vec':
        from baselines.data2vec_baseline import Data2VecTextBaseline
        ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
        model = Data2VecTextBaseline(vocab_size=50304, max_seq_len=512, embed_dim=768, depth=12, num_heads=12)
        model.encoder.load_state_dict(ckpt.get('encoder', {}))
        model.target_encoder.load_state_dict(ckpt.get('target_encoder', {}))
    else:
...
def extract_representations(model, dataloader, max_batches=100, device='cpu',
                            pool='mean'):
...
def run_full_comparison(jepa_model, baseline_model, dataloader,
                        output_dir, device='cpu', max_batches=50):
    ...
    positions = torch.arange(jepa_reps.size(0), dtype=torch.float32).unsqueeze(1).expand(-1, jepa_reps.size(1))
    token_mag = jepa_ids.float()
    token_mag_base = base_ids.float()
    ...
    psi = PolysemanticityIndex(n_clusters_range=(2, 3), n_top_activations=50,
                               n_dimensions_sample=20, device=device)
    ...
    from src.interp.statistical_tests import BootstrapCI, BayesianComparison, EffectSize

    # Compare key metrics with bootstrap
    key_metrics = ['effective_rank_online', 'collapsed_dim_ratio_online',
                   'mean_pairwise_cosine_online', 'sv_entropy_online']
    stat_results = {}
...
def generate_text_summary(results, output_dir):
    ...
    for key in ['jepa_entropy', 'baseline_entropy', 'jepa_total_correlation',
                'baseline_total_correlation', 'jepa_mi_position', 'baseline_mi_position']:
        if key in it:
    ...
    lines.append(f"  Geometry metrics JEPA better: {s.get('n_geometry_jepa_better', '?')}/{s.get('n_geometry_total', '?')}")
...
def main():
    ...
    parser.add_argument('--baseline_ckpt', type=str, required=True, help='Path to baseline checkpoint')
    ...
    results = run_full_comparison(
        jepa_model, baseline_model, dataloader,
        args.output, args.device, args.max_batches
    )
