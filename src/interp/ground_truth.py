...
class SyntheticStructuredModel:
    ...
    def __init__(self, n_samples=500, embed_dim=64, n_classes=5,
                 seq_len=32, snr=5.0, device='cpu'):
    ...
    def generate(self, seed=42):
        ...
        for i in range(N):
            ...
            representations[i, self.class_end:self.position_end] += torch.randn(
                self.position_end - self.class_end) * 0.3
        ...
        return {
            ...
            'depths': depths,
            'ground_truth': {
                'class_dims': list(range(0, self.class_end)),
...
class GroundTruthValidation:
    ...
    def validate_probing_complexity(self):
        ...
        pcc = ProbingComplexityCurve(embed_dim=64, depths=(1, 2, 3),
                                     max_epochs=30, min_accuracy=0.6, device=self.device)
