...
def radar_chart(metrics: Dict[str, float], baseline_metrics: Dict[str, float],
                title="JEPA vs Baseline", labels=None,
                width=600, height=600, output_path=None):
    ...
    lower_is_better = {'anisotropy', 'collapsed_dim_ratio', 'mean_pairwise_cosine',
                       'coherence', 'condition_number', 'total_compression',
                       'mean_psi', 'interference_ratio', 'cv'}
...
def layer_heatmap(layer_data: Dict[str, List[float]],
                  title="Layer-wise Metrics",
                  width=800, height=400, output_path=None):
    ...
    for i, metric in enumerate(metrics):
        ...
        for j, val in enumerate(layer_data[metric]):
            ...
            # Value text
            svg += _svg_text(x + cell_w // 2, y + cell_h // 2 + 4,
                           f'{val:.2f}', size=7, color='black' if norm_val > 0.3 and norm_val < 0.7 else 'white')
...
def bar_chart_with_errors(metric_names: List[str],
                          jepa_means: List[float], jepa_cis: List[Tuple[float, float]],
                          baseline_means: List[float], baseline_cis: List[Tuple[float, float]],
                          title="JEPA vs Baseline", ylabel="Value",
                          width=900, height=500, output_path=None):
    ...
    for i, name in enumerate(metric_names):
        ...
        svg += _svg_line(gx + bar_width // 2, j_lo, gx + bar_width // 2, j_hi, color='#2a5db0', width=2)
        svg += _svg_line(gx + bar_width // 2 - 4, j_lo, gx + bar_width // 2 + 4, j_lo, color='#2a5db0', width=2)
        svg += _svg_line(gx + bar_width // 2 - 4, j_hi, gx + bar_width // 2 + 4, j_hi, color='#2a5db0', width=2)
        ...
        svg += _svg_line(gx + bar_width + 5 + bar_width // 2, b_lo,
                        gx + bar_width + 5 + bar_width // 2, b_hi, color='#b07020', width=2)
...
def probing_complexity_curve(depths: List[int],
                             jepa_accuracies: List[float],
                             baseline_accuracies: List[float],
                             task_name: str = "default",
                             width=600, height=400, output_path=None):
    ...
    if len(jepa_points) > 1:
        for i in range(len(jepa_points) - 1):
            svg += _svg_line(jepa_points[i][0], jepa_points[i][1],
                           jepa_points[i + 1][0], jepa_points[i + 1][1],
    ...
    if len(base_points) > 1:
        for i in range(len(base_points) - 1):
            svg += _svg_line(base_points[i][0], base_points[i][1],
                           base_points[i + 1][0], base_points[i + 1][1],
                           color='#f4a142', width=3)
...
def convergence_plot(steps: List[int],
    ...
    def xp(s): return margin + (s - x_min) / x_range * chart_w
    def yp(v): return margin + chart_h * (1 - (v - y_min) / y_range)
    ...
    for i in range(len(steps) - 1):
        svg += _svg_line(xp(steps[i]), yp(jepa_values[i]),
                        xp(steps[i + 1]), yp(jepa_values[i + 1]),
                        color='#4285f4', width=2)

    # Baseline line
    for i in range(len(steps) - 1):
        svg += _svg_line(xp(steps[i]), yp(baseline_values[i]),
                        xp(steps[i + 1]), yp(baseline_values[i + 1]),
                        color='#f4a142', width=2)
...
def ablation_comparison_chart(ablation_names, metric_name, values,
    ...
    for i, (name, val) in enumerate(zip(ablation_names, values)):
        ...
        svg += _svg_text(margin_left + bar_w + 5, y + bar_height // 2 + 4,
                        f'{val:.3f}', size=8, anchor='start')
...
def scaling_law_plot(sizes, jepa_metrics, baseline_metrics,
                     metric_name="Effective Dimension",
                     jepa_fit=None, baseline_fit=None,
                     width=700, height=450, output_path=None):
    ...
    x_min_v -= x_rng * 0.05; x_max_v += x_rng * 0.05
    y_min_v -= y_rng * 0.05; y_max_v += y_rng * 0.05
    x_rng = x_max_v - x_min_v; y_rng = y_max_v - y_min_v

    def xp(v): return margin + (v - x_min_v) / x_rng * chart_w
    def yp(v): return margin + chart_h * (1 - (v - y_min_v) / y_rng)
...
def robustness_curve(intensities, jepa_cka, baseline_cka,
    ...
    for i in range(len(intensities) - 1):
        svg += _svg_line(xp(intensities[i]), yp(jepa_cka[i]),
                        xp(intensities[i+1]), yp(jepa_cka[i+1]),
                        color='#4285f4', width=3)
    for x, y in zip(intensities, jepa_cka):
        svg += _svg_circle(xp(x), yp(y), 4, fill='#4285f4')

    for i in range(len(intensities) - 1):
        svg += _svg_line(xp(intensities[i]), yp(baseline_cka[i]),
                        xp(intensities[i+1]), yp(baseline_cka[i+1]),
                        color='#f4a142', width=3)
...
def information_plane(mi_input, mi_task, layer_labels=None,
                      title="Information Plane",
                      width=600, height=500, output_path=None):
    ...
    for i in range(len(mi_input) - 1):
        ...
        svg += _svg_line(xp(mi_input[i]), yp(mi_task[i]),
                        xp(mi_input[i+1]), yp(mi_task[i+1]),
                        color=color, width=2)
