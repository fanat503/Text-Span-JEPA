...
class JSpaceMetrics(nn.Module):
    ...
    @torch.no_grad()
    def compute(self, online_h, target_h, predictor_h=None,
                prev_online_h=None):
        metrics = {}
        try:
            ...
            if self._prev_jspace_vectors is not None and self._prev_jspace_vectors.shape == V_jspace.shape:
    ...
    @staticmethod
    def _compute_jspace_quality(spectral_gap, orthogonality, concentration,
                                broadcast, stability,
                                w_gap=0.25, w_ortho=0.2, w_conc=0.2,
                                w_broadcast=0.15, w_stability=0.2):
        try:
            ...
            quality = (w_gap * gap + w_ortho * ortho + w_conc * conc +
                       w_broadcast * bcast + w_stability * stab)
    ...
    def _zero_metrics(self):
        return {
            'jspace_dim_fraction': 0.0, 'jspace_capacity': 0.0,
            'jspace_concentration': 0.0, 'jspace_spectral_gap': 0.0,
            'jspace_broadcast_score': 0.0, 'jspace_orthogonality': 0.0,
            'jspace_stability': 0.0, 'jspace_quality': 0.0,
            'jspace_separation': 0.0,
        }

    def extra_repr(self):
        return (f'variance_threshold={self.variance_threshold}, '
                f'k_workspace={self.k_workspace}')
