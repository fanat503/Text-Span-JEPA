...
### Mechanism #13: STA (Spectral Transport Alignment)

- **Problem**: Spectral Drift Destabilizes Workspace — during training, the eigenvalue spectrum of Cov(z) continuously changes as the encoder learns. This spectral drift causes: (1) JAWP workspace Q oscillates when eigenvalues cross, (2) SPC band allocation becomes stale, (3) downstream representations are non-stationary across training steps.
- **Solution**: Measure and penalize the Wasserstein-1 distance between current and EMA reference eigenvalue spectra: L_STA = η · W_1(λ_current, λ_ref). Theorem (Davis-Kahan + STA): d_Gr(Q_k(t), Q_k(t+1)) ≤ W_1/δ where δ is the spectral gap. STA bounds W_1, thus bounding workspace drift.
- **Novelty**: No prior work monitors eigenvalue spectral drift in self-supervised learning or applies Wasserstein transport distances to stabilize workspace evolution. Related concepts:
  - EMA of model weights (BYOL, I-JEPA): stabilizes the target encoder's weights — STA stabilizes the representation's SPECTRUM, which is orthogonal
  - Learning rate warmup: stabilizes early training dynamics — STA stabilizes spectral evolution throughout training
  - Spectral normalization (Miyato et al., ICLR 2018): constrains spectral norm of weight matrices — STA constrains spectral DISTANCE between consecutive representation covariances
  - Wasserstein GAN (Arjovsky et al., ICML 2017): uses Wasserstein distance for distribution matching — STA uses it for spectral stability monitoring (different object, different purpose)
  - No prior art found for: Wasserstein-1 distance between eigenvalue spectra of representation covariance across training steps for workspace stability
- **Why top labs will use it**: STA provides a principled, lightweight way to ensure smooth spectral evolution. This is critical for any method with learned subspaces (JAWP, PCA, spectral methods) that depend on eigenvalue ordering. The Davis-Kahan bound is constructive and provides a tunable stability guarantee. Costs O(D²) per update_interval steps (default 10), near-free amortized.

---

## Overall Assessment (13 mechanisms)

**All 13 mechanisms are genuinely novel.** None were stolen from prior work. Each either:
1. Introduces a completely new concept (JAWP, WIP, CGN, PCR, SPC, CMC, GAC, STA)
2. Applies existing mathematical tools to a NEW problem in a novel way (Grassmann, Spectral Gap, Predictive Rank)
3. Creates a new combination that addresses a specific JEPA failure mode (SWIP, WSD)
