...
class TestSAE:
    ...
    def test_dead_feature_resampling(self):
        sae = self.SAE(input_dim=32, latent_dim=64, k=8,
                       resample_interval=1)
        x = torch.randn(16, 32)
        for _ in range(5):
            sae(x)
...
class TestSAETrainer:
    def test_train_step(self):
        from src.interp.sae import SparseAutoencoder, SAETrainer
...
class TestStructuralProbe:
    ...
    def test_train_and_evaluate(self):
        ...
        gold = torch.cdist(torch.randn(5, 8, 1), torch.randn(5, 8, 1)).squeeze(-1)
...
class TestCausalIntervention:
    ...
    def test_intervention_predictability_score(self):
        ...
        enc = TextSpanJEPLEncoder(vocab_size=100, max_seq_len=16,
        ...
        result = intervention_predictability_score(
            MockModel(), ids, direction, probe_fn)
...
class TestProbingComplexity:
    ...
    def test_evaluate_single_task(self):
        from src.interp.probing_complexity import ProbingComplexityCurve
        pcc = ProbingComplexityCurve(embed_dim=16, depths=(1, 2),
                                     max_epochs=10, device='cpu')
...
class TestPolysemanticity:
    def test_polysemanticity_index(self):
        ...
        psi = PolysemanticityIndex(n_clusters_range=(2, 3),
...
class TestCollapseNewMetrics:
    ...
    def test_no_nan_new_metrics(self):
        ...
        m = diag.compute(x, y)
        for key in ['svcca_online_target', 'alignment', 'eigenvalue_spread_online',
                     'subspace_overlap', 'spectral_clustering_coeff_online']:
            val = m.get(key, 0)
...
class TestVisualization:
    ...
    def test_bar_chart(self):
        ...
        svg = bar_chart_with_errors(
            names,
            jepa_means=[45, 0.3], jepa_cis=[(43, 47), (0.25, 0.35)],
            baseline_means=[38, 0.6], baseline_cis=[(35, 41), (0.55, 0.65)]
        )
        assert svg is not None
        assert '<svg' in svg
...
class TestAblation:
    def test_ablation_config(self):
        from src.interp.ablation import AblationConfig, ABLATION_CONFIGS
...
class TestScaling:
    ...
    def test_efficiency(self):
        ...
        result = InterpretabilityEfficiency.compute_efficiency(
            interp_metric=0.85, flops=1e18,
            baseline_interp=0.70, baseline_flops=1e18
        )
...
class TestInterpretabilityIndex:
    def test_compute(self):
        ...
        metrics = {'effective_dimension': 40, 'anisotropy': 0.3,
                   'sv_entropy': 0.7, 'frac_monosemantic': 0.5}
    ...
    def test_compare(self):
        from src.interp.interpretability_index import InterpretabilityIndex
        idx = InterpretabilityIndex()
        jepa = {'effective_dimension': 45, 'anisotropy': 0.2, 'sv_entropy': 0.8,
                'frac_monosemantic': 0.6, 'mean_psi': 1.0}
        baseline = {'effective_dimension': 35, 'anisotropy': 0.6, 'sv_entropy': 0.5,
                    'frac_monosemantic': 0.3, 'mean_psi': 2.0}
    ...
    def test_from_collapse(self):
        from src.interp.interpretability_index import InterpretabilityIndex
        collapse = {'effective_rank_online': 40, 'sv_entropy_online': 0.7,
                    'collapsed_dim_ratio_online': 0.1, 'mean_pairwise_cosine_online': 0.3,
                    'uniformity_online': -2.5, 'participation_ratio_online': 20}
